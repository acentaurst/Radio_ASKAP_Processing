"""Download source-specific ASKAP cutout archives selected from the catalogue."""
...
def main() -> None:
        ...
        if os.path.isdir(folder_path):
            ...
            if os.path.exists(longobs_path) and os.path.isdir(longobs_path):
                ...
                for sb_num in unique_sb_numbers:
                    records.append({'Name': folder_name, 'SBID': sb_num})
    # 创建DataFrame
    ...
    for i in range(len(DSinfo['Name'])):
        ...
        pmra = Star['pmra_formula'].item()
        pmdec = Star['pmdec_formula'].item()
        missing_pm = []
        if pd.isna(pmra):
            missing_pm.append('pmra_formula')
            pmra = 0.0
        if pd.isna(pmdec):
            missing_pm.append('pmdec_formula')
            pmdec = 0.0
        if missing_pm:
            print(
