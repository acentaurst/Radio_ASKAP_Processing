...
# 3. 定向提取数据
print(f" 开始处理 [无筛选图]: {target_safe_name} ...")
...
if not plot_data: exit(f" 未提取到有效数据。")
...
df_plot[['SBID', 'MJD', 'Date', 'Flux_I', 'Flux_V', 'V_over_I_raw', 'V_over_I_abs', 'RMS_I', 'RMS_V', 'RMS_VI']].to_csv(
    os.path.join(output_dir, f'{target_safe_name}_Measurements{file_suffix}.csv'), index=False)
...
fig.suptitle(f'Unfiltered Lightcurve: {target_original_name}\n{title_scale} {title_time}', fontsize=17,
             fontweight='bold', y=1.05)

ax1.errorbar(df_plot['MJD'], df_plot['Flux_I'], yerr=df_plot['RMS_I'], fmt='-o', color=color_I, ecolor=color_I,
             capsize=3, linewidth=2, markeredgecolor='white', label='Stokes I')
ax1.errorbar(df_plot['MJD'], df_plot['Flux_V'], yerr=df_plot['RMS_V'], fmt='-s', color=color_V, ecolor=color_V,
             capsize=3, linewidth=2, markeredgecolor='white', label='Stokes V')
ax2.errorbar(df_plot['MJD'], df_plot['V_over_I_abs'], yerr=df_plot['RMS_VI'], fmt='-^', color=color_VI, ecolor=color_VI,
             capsize=3, linewidth=2, markeredgecolor='white', label='|V/I| Ratio')
...
if not df_plot.empty:
    ...
    ax2.set_ylim(-10, max(120, max_vi * 1.1))
...
print(f" 处理完毕！结果已保存至: {output_dir} \n (文件名后缀: {file_suffix})")
