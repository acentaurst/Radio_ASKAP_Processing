...
# --- 控制参数 ---
TARGET_SOURCES: List[str] = ['Proxima Cen']
...
def process_single_tar(tar_path: str, clean_hostname: str, star_meta: Dict[str, Any], sbid: str, beam: str,
    ...
    if not wsclean_done:
        logger.warning(f"️ WSClean 模型未就绪，开始建图 ({WSCLEAN_THREADS} 线程)...")
        ...
        logger.info(f"执行 WSClean 建模 ({WSCLEAN_THREADS} 线程)...")
        os.makedirs(wsclean_model_full_path, exist_ok=True)
        wsclean_cmd = (
            f"wsclean -name {wsclean_model_dir_name}/wsclean -size 8192 8192 -scale 2.5asec "
            f"-niter 1000000 -mgain 0.8 -pol iv -weight briggs 0.5 "
            f"-auto-threshold 5 -auto-mask 6 -multiscale -multiscale-scale-bias 0.7 "
            f"-multiscale-max-scales 8 -join-channels -channels-out 8 -deconvolution-channels 8 "
            f"-fit-spectral-pol 3 -j {WSCLEAN_THREADS} -temp-dir {wsclean_model_dir_name} {clean_ms_name}"
        )
        run_cmd(wsclean_cmd, cwd=workspace_dir)
...
def process_entire_source(clean_hostname: str, tar_files: List[str], star_meta: Dict[str, Any],
    ...
    for tar_path in tar_files:
        ...
        if sbid not in sbid_to_mjd:
            logger.warning(f"无法在 01 表中找到 SBID {sbid} 的对应观测时间，跳过ms数据文件。")
            continue
