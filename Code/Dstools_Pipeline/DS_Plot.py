"""Create server-side dynamic-spectrum, image, and quality-control figures from DS files."""
...
# ==========================================
# 主流程（单文件处理保留为 ThreadPoolExecutor 回调）
# ==========================================
def process_ds_file(ds_file, output_dir):
    ...
    with h5py.File(ds_file, "r") as f:
        time_raw = f["time"][:]
    dur = (time_raw[-1] - time_raw[0]) / 3600.0
    if dur < TAVG_THRESHOLD_HOURS:
        tavg_lc, tavg_ds = TAVG_SHORT, TAVG_SHORT
    else:
        tavg_lc, tavg_ds = TAVG_LONG, TAVG_LONG
    ...
    if not science_done:
        ...
        title_str = f"{hostname} | SB{sbid} | beam {beam}"
        ...
        ax1.errorbar(time_lc, flux_i_plot, yerr=flux_i_err,
                     fmt='-', color="black", linewidth=0.8, capsize=2,
                     alpha=0.85, label="Stokes I")
        ax1.errorbar(time_lc, flux_v_plot, yerr=flux_v_err,
                     fmt='-', color="#e74c3c", linewidth=0.8, capsize=2,
        ...
        add_panel_label(ax1, "Lightcurve", fontsize=12)
        ...
        add_panel_label(ax2, "Stokes I", fontsize=12)
        ...
        add_panel_label(ax3, "Stokes V", fontsize=12)
    ...
    if INCLUDE_MFS:
        if not mfs_done:
            ...
            fits_i_path = os.path.join(fits_dir, "wsclean-MFS-I-model.fits")
            fits_v_path = os.path.join(fits_dir, "wsclean-MFS-V-model.fits")

            # 从星表计算目标坐标；proper-motion 缺失时显式记录警告。
            corr_ra, corr_dec = None, None
            ...
            sbid_to_mjd = (obs_df.dropna(subset=['sbid_clean'])
                           .drop_duplicates(subset=['sbid_clean'])
                           .set_index('sbid_clean')['t_min'].to_dict())
            ...
            star_catalog_dict = (stars_df.drop_duplicates(subset=['hostname_clean'])
                                 .set_index('hostname_clean').to_dict('index'))
            norm_h = re.sub(r'[^a-zA-Z0-9]', '', hostname).lower()
            matched_key = None
            for k in star_catalog_dict.keys():
                if (re.sub(r'[^a-zA-Z0-9]', '', k).lower() in norm_h
                        or norm_h in re.sub(r'[^a-zA-Z0-9]', '', k).lower()):
                    matched_key = k
                    break
            if matched_key:
                ...
                if obs_mjd:
                    ...
                    star_j2015 = SkyCoord(
                        ra=star_meta['ra'] * u.deg, dec=star_meta['dec'] * u.deg,
                        pm_ra_cosdec=pmra * u.mas / u.yr, pm_dec=pmdec * u.mas / u.yr,
                        distance=(1000 / plx) * u.pc, frame='icrs', obstime=Time('J2015.5'))
                    star_at_obs = star_j2015.apply_space_motion(
                        new_obstime=Time(obs_mjd, format='mjd'))
            ...
            fig, axes = plt.subplots(
                2, 1, figsize=(15, 12), facecolor="white",
            ...
            for ax, path, label, cmap in [
                ...
                if os.path.exists(path):
                    ...
                    add_panel_label(ax, label, fontsize=12)
                else:
    ...
    if INCLUDE_QC:
        if not qc_done:
            ...
            title_str = f"{hostname} | SB{sbid} | beam {beam} | QC"
            fig.suptitle(title_str, fontsize=18, fontweight='bold', y=0.98)
            fig.subplots_adjust(top=0.95, bottom=0.03)
            ...
            add_panel_label(ax_lc, "|V|/I", fontsize=12)
            ...
            add_panel_label(ax1, "Im(Stokes I)", fontsize=12)
            ...
            add_panel_label(ax2, "Im(Stokes V)", fontsize=12)
